...
def init_files(trace_type = ' ', task_type='CLASSIFICATION', topo_type='b4', 
    ...
    if task_type =='APPLICATION':
        f1.write(("#flows,iteration,r_threshold,is_correlated,#case_detect\n"))
    else:
    ...
    if task_type =='CLASSIFICATION':
        ...
        f2.write('#flows,iteration,time,r_threshold,'
                        + 'is_correlated,'
...
def run_single(topo_type, task_type, trace_type, iter_start=0, iter_end=10, 
    ...
    if topo_type == "B4":
        if task_type == 'CLASSIFICATION':
            flow_nums = np.arange(100, 810, 100)#np.arange(50, 310, 50)#np.arange(50, 410, 25) #np.arange(675, 860, 50)#np.arange(225, 660, 50)#np.arange(50, 210, 25)
            flow_nums = [500]
    ...
    if topo_type == "TREE":
        if task_type == 'CLASSIFICATION':
            flow_nums = np.arange(100, 810, 100)#np.arange(50, 310, 50)#np.arange(50, 410, 25)#np.arange(225, 660, 50)#np.arange(50, 210, 25)#np.arange(50, 250, 50) #np.arange(200, 2100, 300)
    ...
    if topo_type == "JUPITER":
        if task_type == 'CLASSIFICATION':
            flow_nums = np.arange(100, 810, 100) #np.arange(50, 260, 25)#np.arange(5000, 36000, 2500) # 5000, 30000
    ...
    if x_var == 'BIAS' or 'RTHRESHOLD' in x_var:        
        ...
        if topo_type == 'JUPITER':
            r_thresholds = [x/5.0 for x in r_thresholds]
    ...
    if 'PVALUE' in x_var or 'RTHRESHOLD' in x_var:
        flow_nums = [100] #flow_nums[len(flow_nums)-1:]
        if topo_type == "JUPITER":
            flow_nums = [800]
            ...
            for epsilon in epsilons:
                for r_threshold in r_thresholds:
                    for imr_threshold in imr_thresholds:
                        ...
                        if task_type == 'RANKING':
                            drop_id = 0
                        print '\n@CORRELATED ', '@epsilon', '@flownum, @iteration', epsilon, flow_num, iteration
                        command = ('python mesh_topo_ranking.py ' 
                                            + topo_type + ' ' 
                                            + str(flow_num) + ' '
                                            + str(is_correlated) + ' '
                                            + trace_type + ' '
                        ...
                        """if task_type == "CLASSIFICATION":
                            print '\n@NOT CORRELATED', '@epsilon, @flownum, @iteration', epsilon, flow_num, iteration
...
if __name__ == '__main__':
    ...
    x_vars = ['TTESTFLOWNUM',  'TTESTPVALUE', 'TTESTPVALUEBYTE', 'TTESTRTHRESHOLD', 'TTESTRANKINGPVALUEFLOWNUM', 'BIAS', 'IMR']
    x_vars1 = ['CHERNFLOWNUM','CHERNPVALUE', 'CHERNPVALUEBYTE', 'CHERNRTHRESHOLD', 'CHERNRANKINGPVALUEFLOWNUM']
    
    iter_start, iter_end = 0, 10
    topo_type = topo_types[1]
    x_var = 'TTESTRANKINGPVALUEFLOWNUM'#x_vars[0] #check x_var caution init_file

    for task_type in task_types[1:2]:
        for trace_type in trace_types[0:1]:
            run_single(topo_type, task_type, trace_type, 
