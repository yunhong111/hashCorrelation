...
def next_hop_rand_mark(cur_hop, pre_hop, s, d, hash_str, size,
    ...
    if drop_id > 0 and n > 1 and cur_hop == test_hop and nhop == table[cur_hop][d][0] and len(add_byte_dict) < 2:
        
        add_byte_dict[hash_str] = 1000*size
        size = (w_key)*r_threshold/(1-r_threshold*2)
        #print 'haha', size"""
...
def flow_routing(file_name, seeds, polys, flow_paths,
    ...
    with open(("../inputs/total_bytes"), 'a+') as f:
        for line in f:
            ...
            byte_dict_from_file[key1] = float(data[-1])
            ...
            if(len(flow_dict) < 1824763):
                ...
                if task_type != 'APPLICATION1':
                    if len(select_dict) >= flow_cap: #20000:
                        ...
                        return len(select_dict), imrs, exc_time
                        break
...
def single(r1, r2, pairs, est_pairs, switch_nodes, 
    ...
    select_len = 0
    if task_type != 'APPLICATION1':
        for s in test_nodes[1:2]:
            print '    **** testing node', s
            flow_paths = {}
            select_len, imrs, app_exc_time = flow_routing(
                file_name, seeds, polys, flow_paths, switch_nodes, table, flow_cap,
                topo_type, 
                key=key, class_dict=class_dict, imr_threshold=imr_threshold,
                task_type=task_type, 
                drop_id=drop_id, r_threshold=r_threshold, black_hole=black_hole,
                links=links, test_hop=s, exc_time=app_exc_time
            )
            start_time = time.time()
    ...
    return trues, ests, select_len, est_pairs, true_class,  exc_time, app_exc_time, imrs
...
if __name__ == "__main__":
    ...
    for (r1, r2) in test_pairs:
        ...
        if r1 != r2:
            key = '-'.join(sorted([r1, r2]))
            trues, ests, select_len, est_pairs, true_class_dict[key], exc_time, app_exc_time, imrs = single(
                ...
                links=links,
                metric_type=x_var,
                exc_time=exc_time, app_exc_time=app_exc_time
            )
                
    print(
        'trues, ests, select_len, est_pairs, exc_time, app_exc_time', 
        len(trues), len(ests), select_len, est_pairs, exc_time, app_exc_time
    )
    ...
    if task_type == "CLASSIFICATION":
        ...
        f1.write(
            ','.join([str(x) for x in 
            [select_len, iteration, epsilon, r_threshold, int(is_correlated), len([x for x in true_byte_dict.values() if x[2] == 1]), 
    ...
    if task_type == "RANKING":    
        ...
        f1.write(
            ','.join([str(x) for x in 
            [select_len, iteration, epsilon, r_threshold, int(is_correlated), 
            len(ests)]]
            ) 
            + '\n'
        )
    ...
    if task_type == "APPLICATION":    
        ...
        f1.write(
            ','.join([str(x) for x in 
            [select_len, iteration, r_threshold, imr_threshold, int(is_correlated), 
