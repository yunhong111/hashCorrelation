...
def accuracyPlot(topo_types, task_types, trace_types, pair_seq=0):
    ...
    acc_legend = ['dropping rate = 0.1', 'dropping rate = 0.15', 'dropping rate = 0.2']
    ...
    pl.plot(accuracys, x=xss, k=2, errors=q_75ss, 
        ...
        xlog=False, ylog=False, acc_legend=acc_legend, legend_x=0.3,
        legend_y=0.2
    )
    print len(acc_boxplot_data)

    chern_x = [2*x for x in chern_x]
    pl.box_plot(acc_boxplot_data, x=chern_x, k=2, errors=errors, 
        xlabel=xlabel, ylabel=ylabel, title=infile_name1.split('/outputs')[1].split('.csv')[0].replace('.', '_'), 
