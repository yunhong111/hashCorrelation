...
def retieveData(infile_name1, group_fields, agg_fields, 
                ...
                iteration=10, pair_num=4, key_field='#flows', q_75ss=[]
    ):
    print infile_name1
    ...
    for i in range(iteration*pvalue_num):
        ...
        if df1_part1[key_field].loc[s_index] != df1_part1[key_field].loc[e_index]:
            print df1_part1.loc[s_index], df1_part1.loc[e_index]
            eeee
        key = df1_part1[key_field].loc[e_index]

        y_pred = df1_part1['pred'].loc[s_index:e_index]
        y_pred = y_pred.append(df1_part2['pred'].loc[s_index:e_index])
        y_true = df1_part1['true'].loc[s_index:e_index]
...
def accuracyPlot(topo_types, task_types, c, pair_seq=0):
    ...
    acc_legend = ['B4', 'Fat-tree', 'Jupiter']
