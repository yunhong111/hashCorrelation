...
def read_csv(file_name, file_type=0, topo_type='CLASSIFICATION', is_epsilon=False):
    ...
    if file_type == 0:
        ...
        if is_epsilon:
            names=['#flows', 'epsilon', 'r_threshold', 'is_correlated', 'key'] + ['a' + str(i) for i in range(ACAP)]
...
def retieveData(infile_name1, group_fields, agg_fields, 
                ...
                iteration=10, pair_num=4, key_field='r_threshold', q_75ss=[]
    ):
    print infile_name1
...
def accuracyPlot(topo_types, task_types, trace_types, pair_seq=0):
    ...
    pl.plot(accuracys, x=chern_x, k=2, errors=q_75ss, 
        ...
        xlog=False, ylog=False, acc_legend=acc_legend, legend_x=0.2,
        legend_y=0.2
    )
